...
mask_path = "/mnt/data/label_mask.tif"  # Update with actual mask path
tile_size = 512
output_dir = "tiles/"
...
with rasterio.open(image_path) as src_img:
    ...
    # Open the mask (if available)
    has_labels = os.path.exists(mask_path)
    if has_labels:
        with rasterio.open(mask_path) as src_mask:
            mask_data = src_mask.read(1)  # Read single-channel mask
    ...
    for i in range(0, img_width, tile_size):
        for j in range(0, img_height, tile_size):
            ...
            # Read mask tile if labels exist
            if has_labels:
                mask_tile = mask_data[j:j+tile_size, i:i+tile_size]

                # **Check real content in mask**
                non_zero_mask_pixels = np.sum(mask_tile > 0)  # Count labeled pixels
                tile_area = tile_size * tile_size
                real_content_ratio = non_zero_mask_pixels / tile_area
            else:
                real_content_ratio = 1.0  # Assume full content
            ...
            # **Save mask tile if labels exist**
            if has_labels:
                mask_tile_path = os.path.join(output_dir, "masks", f"tile_{i}_{j}.tif")
                with rasterio.open(
                    mask_tile_path, "w", driver="GTiff", height=tile_size, width=tile_size,
